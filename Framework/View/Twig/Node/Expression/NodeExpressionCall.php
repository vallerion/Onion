...
abstract class NodeExpressionCall extends NodeExpression
{
    private $reflector;
    ...
    protected function getArguments($callable, $arguments)
    {
        ...
        foreach ($arguments as $name => $node) {
            if (!is_int($name)) {
                ...
                throw new ErrorSyntax(sprintf('Positional arguments cannot be used after named arguments for %s "%s".', $callType, $callName));
            }
        ...
        foreach ($callableParameters as $callableParameter) {
            ...
            if (array_key_exists($name, $parameters)) {
                if (array_key_exists($pos, $parameters)) {
                    throw new ErrorSyntax(sprintf('Argument "%s" is defined twice for %s "%s".', $name, $callType, $callName));
                }

                if (count($missingArguments)) {
                    throw new ErrorSyntax(sprintf(
                ...
                $optionalArguments[] = new NodeExpressionConstant($callableParameter->getDefaultValue(), -1);
                ...
                throw new ErrorSyntax(sprintf('Value for argument "%s" is required for %s "%s".', $name, $callType, $callName));
            }
        }

        if ($isVariadic) {
            $arbitraryArguments = new NodeExpressionArray(array(), -1);
            foreach ($parameters as $key => $value) {
                if (is_int($key)) {
                    $arbitraryArguments->addElement($value);
                } else {
                    $arbitraryArguments->addElement($value, new NodeExpressionConstant($key, -1));
                }
