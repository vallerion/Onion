...
//            @trigger_error(sprintf('Passing a string as the $name argument of %s() is deprecated since version 1.27. Pass a Source instance instead.', __METHOD__), E_USER_DEPRECATED);
    ...
    protected function compileTemplate(Compiler $compiler)
    {
        ...
        if (
            ...
            || !$this->hasNode('parent')
            || $this->getNode('parent') instanceof NodeExpressionConstant
    ...
    protected function compileConstructor(Compiler $compiler)
    {
        ...
        } elseif (($parent = $this->getNode('parent')) && $parent instanceof NodeExpressionConstant) {
            $compiler
                ->addDebugInfo($parent)
    ...
    protected function compileIsTraitable(Compiler $compiler)
    {
        ...
        if ($traitable) {
            if ($this->getNode('body') instanceof NodeBody) {
            ...
            foreach ($nodes as $node) {
                ...
                if ($node instanceof NodeBlockReference) {
                    continue;
                }
