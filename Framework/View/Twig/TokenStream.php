...
class TokenStream
{
    ...
    public function next()
    {
        if (!isset($this->tokens[++$this->current])) {
            throw new ErrorSyntax('Unexpected end of template.', $this->tokens[$this->current - 1]->getLine(), $this->source->getName());
        }
    ...
    public function expect($type, $value = null, $message = null)
    {
        ...
        if (!$token->test($type, $value)) {
            $line = $token->getLine();
            throw new ErrorSyntax(sprintf('%sUnexpected token "%s" of value "%s" ("%s" expected%s).',
                $message ? $message.'. ' : '',
    ...
    public function look($number = 1)
    {
        if (!isset($this->tokens[$this->current + $number])) {
            throw new ErrorSyntax('Unexpected end of template.', $this->tokens[$this->current + $number - 1]->getLine(), $this->source->getName());
        }
